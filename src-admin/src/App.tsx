...
import TabSymbolSettings from './Tabs/SymbolSettings';
...
const tabs: {
    ...
    tooltip?: string;
        ...
        {
            name: 'windsymbol_settings',
            title: 'WindSymbol_settings',
        },
        {
            name: 'moonsymbol_settings',
            title: 'MoonSymbol_settings',
        },
...
class App extends GenericApp<GenericAppProps, AppState> {
    ...
    renderMainSettings(): React.JSX.Element {
        ...
        return (
            <TabMainSettings
                ...
                socket={this.socket}
                native={this.state.native as DasWetterAdapterConfig}
    ...
    renderSymbolSettings(): React.JSX.Element {
        if (!this.state.systemConfig) {
            return <div>{I18n.t('Loading...')}</div>;
        }

        console.log("renderSymbolSettings called");
    ...
    renderWindSymbolSettings(): React.JSX.Element {
        if (!this.state.systemConfig) {
            return <div>{I18n.t('Loading...')}</div>;
        }

        console.log("renderWindSymbolSettings called");

        return (
            <TabWindSymbolSettings
                alive={this.state.alive}
                common={this.common || ({} as ioBroker.InstanceCommon)}
                socket={this.socket}
                native={this.state.native as DasWetterAdapterConfig}
    ...
    renderMoonSymbolSettings(): React.JSX.Element {
        if (!this.state.systemConfig) {
            return <div>{I18n.t('Loading...')}</div>;
        }

        console.log("renderMoonSymbolSettings called");

        return (
            <TabMoonSymbolSettings
                alive={this.state.alive}
                common={this.common || ({} as ioBroker.InstanceCommon)}
                socket={this.socket}
                native={this.state.native as DasWetterAdapterConfig}
    ...
    renderTab(): React.JSX.Element {
        ...
        if (this.state.selectedTab === 'symbol_settings' || !this.state.selectedTab) {
            return this.renderSymbolSettings();
        }
        if (this.state.selectedTab === 'windsymbol_settings' || !this.state.selectedTab) {
            return this.renderWindSymbolSettings();
        }
        if (this.state.selectedTab === 'moonsymbol_settings' || !this.state.selectedTab) {
            return this.renderMoonSymbolSettings();
        }
